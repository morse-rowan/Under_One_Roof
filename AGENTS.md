...
- The official `Roblox_Studio` MCP is configured locally. Run `py -3 tools/check_studio_mcp.py` for a read-only connectivity check; setup/recovery lives in `wiki/project/agent-integrations.md` in the shared vault. Prefer MCP for live engine inspection and targeted playtests. Discover the Studio ID each session, confirm the intended place and play state, and coordinate one writer. Keep Rojo-owned script edits on disk; MCP availability does not change source ownership.
