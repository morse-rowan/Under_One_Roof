# Stores a Roblox Open Cloud API key for this Windows user only, outside the repository.
...
$keyPath = Join-Path $keyDirectory 'roblox-api-key.dpapi'

$entered = Read-Host -AsSecureString 'Paste your Roblox Open Cloud API key (hidden)'
$pointer = [IntPtr]::Zero
try {
...
Write-Host "  $keyPath"
Write-Host 'Next: .\tools\publish_place.ps1 -Universe <id> -Place <id> -Offline'
