...
Sign into Studio, reopen it after plugin installation, and open `build/RoommateDev.rbxlx`. With a place open, select the **Plugins** ribbon tab and click the red **Rojo** button; this local plugin's toolbar is the useful entry point, even if it is absent from Plugin Manager. Connect to `localhost:34872`, inspect the proposed sync changes, and accept them for this development place. Press Play (F5). The current build opens directly into the apartment visual sandbox. Expect `[Roommate] Server ready` / `Client ready` messages without project script errors in Output. Stop play with Shift+F5 before syncing structural changes. Stop the foreground sync server with Ctrl+C.

The default build is a movement-only visual trial: warm kitchen/lounge, square floor cells, simple beans and short hops. WASD/arrows follow the room grid; tap once for one tile or hold to repeat. Walk right along the front row through the doorway to the lounge; walk left through it to return. Press **1** for the baseline warm palette, **2** for lighter warm, and **R** to return to the starting area. There is no game HUD, chore interaction, timer, dialogue or reaction animation. The scene is generated when Play starts, so the edit-time development floor is expected. See the [visual sandbox handoff](<../../Documents/Obsidian Vault/school/Steelhacks/wiki/project/visual-sandbox.md>).
